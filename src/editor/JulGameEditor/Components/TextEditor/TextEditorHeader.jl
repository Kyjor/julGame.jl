...
mutable struct TextEditor
    ...
    # State
    undoIndex::Int
    undoBuffer::Vector{Any}  # Placeholder for UndoRecord type
    lines::Vector{Vector{Char}}
    firstVisibleLine::Int
    lastVisibleLine::Int

    function TextEditor()
        new(false, true, true, true, false, false, 4, 1.0, 0, [], [], 0, 0)
    end
end

# Enums
@enum PaletteId Dark Light Mariana RetroBlue
@enum LanguageDefinitionId None Cpp C Cs Python Lua Json Sql AngelScript Glsl Hlsl
...
function setShowWhitespacesEnabled(editor::TextEditor, value::Bool)
    editor.showWhitespaces = value
end

function isShowWhitespacesEnabled(editor::TextEditor)
    return editor.showWhitespaces
end
...
function setLineSpacing(editor::TextEditor, value::Float64)
    editor.lineSpacing = value
end

function getLineSpacing(editor::TextEditor)
    return editor.lineSpacing
end
